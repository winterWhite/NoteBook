...
正则表达式里的条件用？来定义，简单的？匹配前一个字符或表达式（如果它存在的话），？=和?<=匹配前面或后面的文本（如果它存在的话）。
...
回溯引用条件只在一个前面的子表达式搜索取得成功的情况下才允许使用一个表达式。使用方法如下：

	?(N)regExp
上面这个表达式解释如：?(N)表示引用条件，意思是如果前面第N个子表达式(回溯引用)存在，则使用后面的正则表达式继续进行匹配，否则就不进行匹配。

上面这种表达式在给定条件得到满足时才执行一个表达式，此外，还有否则表达式，否则表达式只在给定回溯引用不存在时才会被执行。使用方法如下：

	?(N)true-reg|false-reg
上面的表达式意思是：如果前面第N个子表达式(回溯引用)存在，则使用true-reg表达式进行匹配，否则使用false-reg表达式进行匹配。

**前后查找条件**

前后查找条件只在一个向前查找或向后查找取得成功的条件下才允许使用一个表达式。使用方法与回溯引用条件大同小异，只需要把?后面括号中的N变为向前查找或向后查找表达式即可。
